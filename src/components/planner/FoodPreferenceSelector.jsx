  ...
  return (
    <section className={`food-preference${compact ? " compact" : ""}`} aria-labelledby={compact ? "modal-food-title" : "food-title"}>
      <div className="food-preference-heading">
        ...
        <span>여러 개를 선택하면 식사 일정과 주변 식당 추천에 함께 반영해요.</span>
      </div>
      <div className="food-preference-options">
        ...
        {foodPreferenceOptions.map((option) => (
          <button type="button" key={option.code} className={value.includes(option.code) ? "active" : ""} onClick={() => toggle(option.code)} aria-pressed={value.includes(option.code)}>
