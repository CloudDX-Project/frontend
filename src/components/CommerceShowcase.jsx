...
function InfiniteProductRow({ id, eyebrow, title, description, linkLabel, items, horizontal = false }) {
        ...
        >
          {clones.map((item, itemIndex) => (
            <article className="commerce-card" key={`${item.id}-${itemIndex}`}>
              <img src={item.image} alt={`${item.location} ${item.title}`} loading="lazy" decoding="async" />
...
export default function CommerceShowcase() {
  return (
    <section className="commerce-showcase" aria-label="여행 상품 추천">
      <InfiniteProductRow id="commerce-tours" eyebrow="TRENDING EXPERIENCES · 12" title="사진만 봐도 떠나고 싶은 투어 & 액티비티" description="세계 곳곳의 버킷리스트 경험을 4초마다 새롭게 만나보세요." linkLabel="투어" items={tourProducts} />
      <InfiniteProductRow id="commerce-stays" eyebrow="LIMITED STAY DEALS" title="예산 방어 필수! 마감 임박 타임세일 숙소" description="여행의 분위기와 예산을 모두 지키는 인기 숙소를 모았어요." linkLabel="숙소" items={saleStays} horizontal />
